...
If deeper company research is useful, recommend running `/cicerone deep` separately after the evaluation.
...
**Hard rule (mandatory):** this signal never fetches or scrapes anything — no WebFetch, no WebSearch, no Playwright against `official_source.url`; cicerone stays zero-fetch here by design, same as the agency-licensing signal. It reads the JD text the mode already has and the candidate's own jurisdiction from `config/profile.yml`.
...
After saving the report and recording in the tracker, append a cover letter draft to the report file under `## Cover Letter Draft`. This is a starting point — not the final letter. The user completes it via `/cicerone cover {slug}`.
...
> Draft generated at evaluation time. Complete via `/cicerone cover {slug}` to fill in angles, confirm research, and generate the PDF.
...
---
*Run `/cicerone cover {slug}` to complete angles, confirm company research, and generate the PDF.*
```
