...
Works in two modes:
- **Slug mode:** `/hwadu cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/hwadu cover` or JD pasted directly — starts from scratch
...
When invoked as `/hwadu cover {slug}`:
