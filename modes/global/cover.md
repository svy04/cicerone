...
Works in two modes:
- **Slug mode:** `/cicerone cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/cicerone cover` or JD pasted directly — starts from scratch
...
When invoked as `/cicerone cover {slug}`:
