...
- **섹션 제목은 한국어로 냅니다.** 렌더러의 기본값이 영어이므로 payload 의 `sections` 에 아래를 넣습니다. 한국 회사에 내는 서류에 "Professional Summary"가 찍히면 안 됩니다
  ...
  영문 이력서를 만들 때는 이 칸을 비우면 기본 영어 제목이 나옵니다.
- **`lang` 은 `ko`** 로 넣습니다. 서식의 한글 글꼴 규칙이 이 값으로 걸립니다. 빠뜨리면 한글이 깨질 수 있습니다(실측 2026-08-21: `lang: ko` 로 뽑은 PDF에 맑은 고딕이 임베드되고 문자 매핑도 들어갔습니다)
...
20. **Hiring-manager audit — off by default, opt-in only.** Run `modes/pdf/hm-audit.md` if and only if one of these is true; otherwise skip straight to Step 21 without prompting.
    - The invocation carried `--hm-audit` (`/cicerone pdf --hm-audit`, or the same flag on a natural-language request).
...
- Or run `/cicerone cover {slug}` later
```
