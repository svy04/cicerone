# 모드: 공고 평가 — Block A부터 G까지

후보자가 채용 공고를 붙여넣으면(주소든 본문이든) 항상 A부터 G까지 일곱 블록을 만들고, 마지막에 지원 결정을 한 화면으로 요약합니다.

<!-- 글자 H 는 auto-pipeline 이 쓰는 지원서 답변 초안 절이 이미 차지하고 있어 비워 둡니다. -->
...
5. **초안 작성**: `modes/cover.md` 로 넘겨 문항마다 글자 수를 맞춘 초안을 만듭니다. 고쳐 쓰면 좋은 자리를 함께 표시합니다

한국 기업이 인공지능 작성 여부를 검사하고 있다는 사실은 세션에서 한 번 알립니다(고용노동부·한국고용정보원 「2023년 하반기 기업 채용동향조사」, 매출 500대 기업 315개소 — 확인 시 감점 42.2%·불합격 23.2%). 초안을 본인 말로 고쳐 쓰기를 권하되, 판단은 후보자가 합니다.
...
**마감일은 Notes 칸에 구획으로 적습니다.** 공고에 마감일이 있으면 `due: 2026-09-20` 형태로, 다른 메모 뒤에 `;` 로 나누어 붙입니다(`판교, 핀테크; due: 2026-09-20`). 이렇게 적어야 `node deadlines.mjs` 가 읽습니다. 문장 안에 섞어 쓰면("마감이 9월 20일이라고 함") 읽지 않습니다 — 보통 문장에 든 날짜를 마감으로 오인하지 않기 위해서입니다.

공고에 마감일이 없으면 적지 않습니다. 지어내지 않습니다.
