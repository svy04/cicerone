...
> "No active applications to follow up on. Apply to some roles first with `/cicerone` and come back when they're aging."
...
- Suggest the user run `/cicerone contacto {company}` to find the right person first
...
> - Trying a different contact via `/cicerone contacto`
...
**To:** {email or "No contact found — run `/cicerone contacto` first"}
...
## 한국 공채일 때 — 기본 주기를 쓰지 않습니다

위 기본값(지원 7일 뒤 연락, 7일마다, 두 번이면 포기 표시)은 수시 채용을 전제한 값입니다. **한국 공채에 그대로 쓰면 정상 절차 중에 포기 표시가 붙습니다.**

2025년 하반기 삼성전자 3급 신입공채의 실제 일정은 직무적합성(서류) 9월 → GSAT 10월 → 면접 11월이었습니다. 단계 사이가 한 달입니다. 지원하고 7일 뒤에 연락할 자리가 아닙니다.

**공채로 판정된 지원은 이렇게 다룹니다.**

| 상황 | 무엇을 하나 |
|---|---|
| 발표 예정일이 공고에 있다 | 그 날짜까지 연락하지 않습니다. 지나고 나서 문의합니다 |
| 발표 예정일이 없다 | 다음 단계가 통상 한 달 뒤라는 것을 알리고, 기다리는 동안 다음 전형(인적성·코딩테스트)을 준비하도록 `modes/written-test.md` 로 보냅니다 |
| 포기 표시 | 공채는 자동으로 붙이지 않습니다. 전형이 끝났다는 근거(불합격 통보·채용 종료 공지)가 있을 때만 붙입니다 |

**수시 채용은 기본값을 씁니다.** 개발 직군 수시는 회사가 정한 절차가 짧고 회신도 빠릅니다.

트랙 판정은 `modes/_shared.md` 가 합니다. 트랙을 모르면 기본값을 쓰되, 그 사실을 후보자에게 한 줄로 알립니다.

**근거**: 삼성전자 공식 채용 페이지 기준 3급 신입공채 절차와 2025 하반기 실제 일정. 다른 대기업도 골격이 같습니다(SK는 SKCT, LG는 LG Way Fit). 단계별 통과율은 기업이 공개하지 않아 **확인하지 못했습니다.**
