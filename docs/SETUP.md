...
If you are using Codex, start the interactive session with `codex`. Slash commands are not guaranteed in Codex, so use the same mode names in a prompt if `/hwadu` is unavailable:
...
| Search for offers | `/hwadu scan` or ask the agent to run `scan` |
| Process pending URLs | `/hwadu pipeline` or ask the agent to run `pipeline` |
| Generate a PDF | `/hwadu pdf` or ask the agent to run `pdf` |
| Draft application email | `/hwadu email` or ask the agent to run `email`; draft-only, never sends, submits, or clicks |
| Batch evaluate | `/hwadu batch` or use `codex exec "Run career-ops batch mode ..."` |
| Check tracker status | `/hwadu tracker` or ask the agent to run `tracker` |
| Fill application form | `/hwadu apply` or ask the agent to run `apply` |
