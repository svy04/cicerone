...
1. In a terminal (one time), clone career-ops **and install its dependencies** — Cowork's local shell has no npm network access, so do this before opening the folder:
   ```bash
   git clone https://github.com/santifer/career-ops.git ~/hwadu
   cd ~/hwadu && npm install
   ```
2. Install [Claude Cowork](https://claude.com/download) and, in **Colaborar/Collaborate** mode, add the `~/hwadu` folder.
